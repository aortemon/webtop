...
NetworkSnapshot NetworkAnalizer::Compute(
  ...
  NetworkSnapshot snapshot = {.rx_mbps=0.0, .tx_mbps=0.0, .rx_bytes_total=0, .tx_bytes_total=0};

  for (const auto &iface : current) {
    if (reader::NetworkReader::IsInterfaceDestDefault(iface.name)) {
  ...
  snapshot.rx_mbps = static_cast<double>(snapshot.rx_bytes_total - prev_stats_.rx_bytes_total) * 8
                     / (interval_seconds * 1000000);
  snapshot.tx_mbps = static_cast<double>(snapshot.tx_bytes_total - prev_stats_.tx_bytes_total) * 8
                     / (interval_seconds * 1'000'000);

  prev_stats_ = snapshot;
  return snapshot;
}

void NetworkAnalizer::Reset() {
  first_run_ = true;
}
