...
Модуль предоставляет набор классов для чтения и парсинга системной информации из
файлов procfs (Linux /proc). Включает утилиты для работы с файлами, парсеры CPU,
памяти, дисков, сети, процессов, загрузки системы и файлов окружения (.env).
...
**Метод**: `ReadAll()` - возвращает вектор `DiskStats` для всех физических
дисков (исключает виртуальные ФС, loop-устройства, RAM-диски).
...
- `IsInterfaceDestDefault(iname)` - проверяет, является ли интерфейс маршрутом
  по умолчанию
...
**Метод**: `ReadAll(path)` - возвращает `std::map<std::string, std::string>` с
парами ключ=значение. Игнорирует пустые строки, комментарии (начинающиеся с #) и
строки без ровно одного знака '='.
